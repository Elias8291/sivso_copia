...
import { Plus, Edit3, Trash2, CheckCircle2, Loader2, AlertTriangle, Package } from 'lucide-react';
...
function FormModal({ item, onClose }) {
    const isEdit = !!item;
    ...
    const handleGuardar = () => {
        if (!form.no_partida) { setErrors({ no_partida: 'Requerido.' }); return; }
        setSaving(true);
        const opts = {
        ...
        if (isEdit) {
            router.put(route('partidas.update', item.id), form, opts);
        } else {
            router.post(route('partidas.store'), form, opts);
        }
    };
        ...
        <div className="relative z-[100]" role="dialog" aria-modal="true">
            ...
            <div className="fixed inset-0 z-[101] w-screen overflow-y-auto">
                <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    <div className="relative w-full transform overflow-hidden rounded-2xl bg-white text-left shadow-xl dark:bg-zinc-900 sm:my-8 sm:max-w-lg border border-zinc-200/50 dark:border-zinc-800/50">
                        <div className="bg-white px-6 pb-4 pt-6 dark:bg-zinc-900">
                            ...
                            <div className="mt-5 space-y-4">
                                <div>
                                    <label className={labelCls}>No. Partida *</label>
                                    <input type="number" value={form.no_partida} onChange={set('no_partida')} className={inputCls} min="1" placeholder="Ej. 244" />
                                    {errors.no_partida && <p className="mt-1 text-xs text-red-500">{errors.no_partida}</p>}
                                </div>
                                <div>
                                    <label className={labelCls}>Descripción</label>
                                    <textarea value={form.descripcion} onChange={set('descripcion')} rows={3} className={inputCls} placeholder="Descripción de la partida…" />
...
export default function Index({ partidas = [], filters = {} }) {
    ...
    const handleDelete = () => {
        ...
        setDeleting(true);
        router.delete(route('partidas.destroy', deleteItem.id), {
            preserveScroll: true,
        ...
        <AuthenticatedLayout
            header={
                <div className="flex items-center gap-2 text-sm font-medium text-zinc-500 dark:text-zinc-400">
                    ...
                    <span className="text-zinc-900 dark:text-zinc-100">PARTIDAS</span>
                </div>
            }
        >
            <Head title="Partidas" />
            ...
            <div className="mx-auto w-full max-w-[1200px] space-y-6">
                ...
                <div className="w-full sm:max-w-md">
                    <SearchInput
                        value={buscar}
                        onChange={setBuscar}
                        placeholder="Buscar por no. partida o descripción…"
                    />
                </div>
                        ...
                        <table className="min-w-[600px] w-full text-left">
                            <thead>
                                <tr className="border-b border-zinc-200 bg-zinc-50/50 dark:border-zinc-800 dark:bg-zinc-900/50">
                                    <th className="px-6 py-3 text-[10px] font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">No. Partida</th>
                                    <th className="px-6 py-3 text-[10px] font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Descripción</th>
                                    <th className="px-6 py-3 text-[10px] font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Productos</th>
                                ...
                                {rows.map((item) => (
                                    <tr key={item.id} className="group transition-colors hover:bg-zinc-50/80 dark:hover:bg-zinc-900/80">
                                        <td className="px-6 py-3">
                                            <span className="inline-flex items-center justify-center rounded bg-zinc-100 px-2.5 py-1 text-sm font-bold tabular-nums text-zinc-800 dark:bg-zinc-800 dark:text-zinc-200">
                                                {item.no_partida}
                                            </span>
                                        </td>
                                        ...
                                        <td className="px-6 py-3">
                                            <span className="inline-flex items-center gap-1.5 text-xs font-semibold text-zinc-700 dark:text-zinc-200">
                                                <Package className="size-3.5 text-zinc-400" strokeWidth={2} />
                        ...
                        {rows.map((item) => (
                            <div key={item.id} className="space-y-1.5 p-4">
                                <div className="flex items-center gap-2">
                                    <span className="rounded bg-zinc-100 px-2 py-0.5 text-sm font-bold tabular-nums text-zinc-800 dark:bg-zinc-800 dark:text-zinc-200">
                                        {item.no_partida}
                                    </span>
                                        ...
                                        <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                            ...
                                            <div className="mt-2">
                                                <p className="text-sm text-zinc-500 dark:text-zinc-400">
                                                    ¿Eliminar partida <span className="font-bold text-zinc-700 dark:text-zinc-300">{deleteItem.no_partida}</span>?
