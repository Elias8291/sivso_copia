import pathlib
path = pathlib.Path(r"c:\Users\Elias\Documents\sistema_sivso_2026\database\migrations\2026_03_30_000000_create_sivso_normalized_schema_on_copiasivso.php")
...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::connection($c)->create('dependencias', function (Blueprint $table) {
            ...
            $table->timestamps();
            $table->unique(['ur', 'nombre'], 'dependencias_ur_nombre_unique');
        });
        ...
        Schema::connection($c)->create('productos', function (Blueprint $table) {
            $table->id();
            $table->foreignId('partida_id')->constrained('partidas')->cascadeOnDelete();
            $table->foreignId('tipo_partida_especifica_id')->constrained('tipos_partida_especifica')->restrictOnDelete();
        ...
        Schema::connection($c)->create('cupos_presupuesto', function (Blueprint $table) {
            ...
            $table->foreignId('tipo_partida_especifica_id')
                ->constrained('tipos_partida_especifica')
                ->restrictOnDelete();
            $table->unsignedSmallInteger('anio');
